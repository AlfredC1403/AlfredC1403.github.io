...
function sidebarColor(a) {
  ...
  gradientClasses.forEach(cls => navLink.classList.remove(cls));
  navLink.classList.add('bg-gradient-' + color);

  var navButton = document.querySelector(".btn");
  gradientClasses.forEach(cls => navButton.classList.remove(cls));
  navButton.classList.add('bg-gradient-' + color);

  

  // Update chart background color
  if (window.dailySalesChart) { // Ensure the chart instance exists
    const newColor = colorMapping[color] || "#43A047"; // Default to success green
    window.dailySalesChart.data.datasets[0].backgroundColor = newColor;
    window.dailySalesChart.update();
  }
  if (window.anualSalesChart) { // Ensure the chart instance exists
    const newColor = colorMapping[color] || "#43A047"; // Default to success green
    window.anualSalesChart.data.datasets[0].borderColor = newColor;
    window.anualSalesChart.data.datasets[0].pointBackgroundColor = newColor;
    window.anualSalesChart.update();
  }
}
